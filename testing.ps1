...
Function Get-SystemInformation {
    ...
    # Check if the network adapter is Ethernet or Wi-Fi and get the IP address
    If ($networkAdapter.Name -like "*Ethernet*") {
        Write-Host "Network Interface: Ethernet"
        $ipAddress = (Get-NetIPAddress -InterfaceAlias $networkAdapter.Name -AddressFamily IPv4 | Select-Object -First 1).IPAddress
    } 
    elseif ($networkAdapter.Name -like "*Wi-Fi*") {
        Write-Host "Network Interface: Wi-Fi"
        $ipAddress = (Get-NetIPAddress -InterfaceAlias $networkAdapter.Name -AddressFamily IPv4 | Select-Object -First 1).IPAddress
    } 
    else {
        Write-Host "Network Interface: Not connected via Ethernet or Wi-Fi"
    }
